...
int nadiPrviPrazni(uint8_t okviri[][64])
{
    for (int i = 0; i < M; i++)
    {
        bool empty = true;
...
int main()
{
    ...
    srand(time(NULL));
    proces procesi[N];
    uint8_t okviri[M][64] = {0};
    uint8_t disk[N][16][64];
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < 16; j++)
        {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            { // provjeravamo jeli na 6 bitu nula
                ...
                if (indexOkvir == -1)
                {
                    ...
                    for (int j = 0; j < velicinaOkvira; j++)
                    {
                       disk[indexIzbacenog][indexRedakTablice][j] = okviri[indexOkvir][j]; // spremanje stranice na disk
                    }
                }
